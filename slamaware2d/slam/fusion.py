...
def find_correspondences(
    ...
    frame_projected_map = project_map_to_frame(
        pc_points, pc_normals, pc_colors, pc_ccounts, h, w, K, transform
    )
    ...
    best_inds = find_best_unique_correspondences(
        valid_img_inds, valid_ccounts, valid_points
    )
...
def are_points_close(
    frame_points: torch.Tensor, map_points: torch.Tensor, dist2_th: float
):
...
def are_normals_similar(
    frame_normals: torch.Tensor, map_normals: torch.Tensor, dot_th: float
):
...
def get_alpha(
    points: torch.Tensor,
    sigma: Union[int, float, torch.Tensor],
    dim: int = 2,
    eps: float = 1e-7,
):
