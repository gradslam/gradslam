...
def solve_linear_system(A: torch.Tensor, b: torch.Tensor, damping_coefficient: float = 0.0):
...
def gauss_newton_solve(src_pc: torch.Tensor, tgt_pc: torch.Tensor, tgt_normals: torch.Tensor):
...
def point_to_plane_ICP(
    ...
    for it in range(numiters):
        ...
        _, one_step_b, chamfer_indices_onestep = gauss_newton_solve(one_step_pc, tgt_pc, tgt_normals)
...
def ICP(global_map, new_frame, initial_transform, grad_LM, ds_ratio):
    ...
    ref_pointcloud = global_map.last_proj_vertex_map[::ds_ratio, ::ds_ratio].reshape(-1, 3).unsqueeze(0)
    ref_pc_normals = global_map.last_proj_normal_map[::ds_ratio, ::ds_ratio].reshape(-1, 3)
    ...
    if grad_LM:
        transform, correspondence = point_to_plane_ICP_smooth(
            new_pointcloud, ref_pointcloud, ref_pc_normals, initial_transform, h_ds, w_ds,
        )
    else:
