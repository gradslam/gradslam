...
class CfgNode(dict):
    ...
    def __init__(
        self, init_dict: Optional[dict] = None, key_list: Optional[list] = None, new_allowed: Optional[bool] = False
    ):
        r"""
        Args:
    ...
    def __setattr__(self, name: str, value):
        ...
        _assert_with_logging(
            name not in self.__dict__, "Invalid attempt to modify internal CfgNode state: {}".format(name),
        )
    ...
    def merge_from_list(self, cfg_list: list):
        ...
        """
        _assert_with_logging(
            len(cfg_list) % 2 == 0, "Override list has odd lengths: {}; it must be a list of pairs".format(cfg_list),
        )
        root = self
    ...
    def register_renamed_key(self, old_name: str, new_name: str, message: Optional[str] = None):
    ...
    def _load_cfg_from_file(cls, file_obj):
        ...
        if file_ext in _YAML_EXTS:
            ...
            raise Exception(
                "Attempt to load from an unsupported filetype {}; only {} supported".format(_YAML_EXTS.union(_PY_EXTS))
            )
    ...
    def _load_cfg_py_source(cls, filename):
        ...
        _assert_with_logging(
            hasattr(module, "cfg"), "Python module from file {} must export a 'cfg' attribute".format(filename),
        )
        VALID_ATTR_TYPES = {dict, CfgNode}
        _assert_with_logging(
            type(module.cfg) in VALID_ATTR_TYPES,
            "Import module 'cfg' attribute must be in {} but is {}".format(VALID_ATTR_TYPES, type(module.cfg)),
        )
...
def _merge_a_into_b(a: CfgNode, b: CfgNode, root: CfgNode, key_list: list):
    ...
    """
    _assert_with_logging(
        isinstance(a, CfgNode), "`a` (cur type {}) must be an instance of {}".format(type(a), CfgNode),
    )
    _assert_with_logging(
        isinstance(b, CfgNode), "`b` (cur type {}) must be an instance of {}".format(type(b), CfgNode),
    )
